...
class DatabasePM(Frame):
    ...
    def connectWeb(self,keyword):
        conn = sqlite3.connect('user_data.db')
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM user_data WHERE website LIKE '%{keyword}%'")
    ...
    def connectID(self,keyword):
        conn = sqlite3.connect('user_data.db')
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM user_data WHERE username LIKE '%{keyword}%'")
        record = cursor.fetchall()
        conn.close()
        return record
    ...
    def deleteSQL(self,website,username,password):
        conn = sqlite3.connect('user_data.db')
        cursor = conn.cursor()
        cursor.execute(f"DELETE FROM user_data WHERE website LIKE '%{website}%' AND username LIKE '%{username}%'")
        conn.commit()
        conn.close()
